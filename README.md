# AIGameTranslater
Сделаю предупреждение сразу! Это очень нишевая тема, нужно хотя бы базовое знание нейросетей и как они работают. Я попытался расписать так, что бы даже человек, который в этом не разбирается что-то да понял. Ну и сразу скажу, что ПК нужен довольно мощный. У меня всё крутиться без проблема на 4070 Ti Super с хорошими моделями (Qwen-2B-VL для вытаскивания текста и Gemma-12B для самого перевода), так я получаю качественный перевод. Так же так как я увлекся это всей темой, у меня стоит второй ПК с 3090, там всё еще слаще, более тяжелые модели переводят текст независимо какая игра/программа у вас запущена, потому что игра на основном ПК, а нейросеть работает на втором.  На самом деле довольно уникальная программа, я таких еще не видел, что бы всё крутилось на локальном ПК, это позволяет пользоваться ей даже, если нет интернета, а так же без задержек (вся задержка будет зависить от вашего ПК).

Сделано мной совместно с ~~Чатом ГП~~ , Gemini. Козлы на GPT забанили мне аккаунт(((.
...
Локальный AI переводчик использующий ресурсы ПК для перевода игр и другого рода контента. Работает по принципу: Раз в 1-2 секунды (можно настроить) делает скриншот области -> отправляет скриншот нейросети -> нейросеть распознает текст и переводит на русский -> выводит текст на экран (Profit)
...
- GPU: Не требуется (Можно запустить на процессоре).
- ОЗУ: 4–8 ГБ.
...
А вы что думали, будет все радужно?
Конечно есть аналоги более щадящие по железу (хотя с легкими моделями будет примерно тоже самое), я и сам изначально пробовал его сделать, но мне абсолютно не понравилось как оно переводит и как работает впринципе. Почти всегда текст похожий на "Она - со мной, углепластик...", ну вы поняли. Распознавание текста тоже хромает так еще и на GPU завести проблематично, а на CPU я видел нагрузку 102% (это не шутка, незнаю как диспетчер выдал мне такое)... Поэтому вариант с AI меня более чем устраивает. (В самом низу покажу нагрузку и что у меня стоит)
...
- SOLO: Одна нейросеть делает всё (читает картинку и переводит). Качественно, понимает контекст, но очень тяжело для видеокарты.
<img width="875" height="112" alt="image" src="https://github.com/user-attachments/assets/c12bf9bd-2ca8-4906-9e2d-4d7397dea2b1" />


- DUAL: Работают две модели. Маленькая читает текст, большая переводит. Быстрее, чем Соло, гибче в настройке.
...
- Сжатие кеша (Так же снижает раход, модели могут немного поглупеть, но совсем немного)
