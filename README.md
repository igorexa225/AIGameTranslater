...
Конечно есть аналоги более щедящие по железу (хотя с легкими моделями будет примерно тоже самое), я и сам изначально пробовал его сделать, но мне абсолютно не понравилось как оно переводит и как работает впринципе. Почти всегда текст похожий на "Она - со мной, углепластик...", ну вы поняли. Распазнование текста тоже хромает так еще и на GPU завести проблематично, а на CPU я видел нагрузку 102%... Поэтому вариант с AI меня более чем устраивает. 
Моё железо:
- I7 12700KF
- 32 ГБ ОЗУ
- RTX 4070 Ti Super
...
2) Скачайте VL (vision, с mmproj) модель формата .GUFF и модель mmproj тоже формата .GUFF (она должна идти в "комплекте" с моделью, если нет ищите другую, ОБЯЗАТЕЛЬНО что бы mmproj был от той модели, которую вы качаете)
3) Как скачаете переходите к установке

УСТАНОВКА
1) Скачать
2) Распаковать в удобное место (желательно на SSD и в папку без русских символов)
3) Закунуть модели по пути "GameTranslator\_internal\models\llm"
...
3) Кнока "Начать перевод" запускает сервер с AI и начинает захватывать и переводить текст под областью
4) Окно для захвата - выберите процесс игры или необходимой программы
5) Если вы правильно установили модели, то они сами подхватятся в программе. (если нет проверяйте путь, возможно неправильно поставили)
6) Лор файл - небольшой сборник терминов для нейросети, что бы она правильно переводила имена (определяла их пол, если персонаж женщина, то подстраивала перевод под нее), термины, фракции, организации и т.д. Можете сделать сами или с помощью той же онлайн нейросети попросить создать его (напишите примерно: Дай краткое описание всех персонажей (пол и т.д), терминов, фракций, организаций и т.д игры "название игры", а так же перевод всего этого с английского на русский (или другой интересующий вас язык, перевод с поднебесной и других языков поддерживается). Если лора нет, остаьте пустым (перевод может ухудшиться). По умолчанию находится в "GameTranslator\_internal\assets"
7) Смена шрифта, его размера и толщина - тут всё просто (не спрашивайте почему там "гарнитура" я сам не знаю), там все шрифты в вашей система, выбирайте любой и подгоняйте размер как вам нужно.
...
tempperature - "креативность" AI
top_p - та же "креативность", но в другом плане.
9) Доп. параметры генерации
...
Плавный вывод - если включен = режим новелы, текст появляется плавно (скорость меняется стрелочками вверх или вних), если выключен = появляется сразу по мере поступления текста
...
13) Настроить промпт - позволяет в ручную настраивать промпт для нейросети.
14) Предпросмотр промпта - показывает текущий промпт. Мной уже загружен промпт, можете использовать его как шаблон для своего. ГЛАВНОЕ ЕСЛИ ВЫ МЕНЯЕТЕ ПРОМПТ И ИСПОЛЬЗУЕТЕ ФАЙЛ ЛОРА ТО В КОНЦЕ ЯБАЗАТЕЛЬНО СДЕЛАЙТЕ ТАК, ИНАЧЕ ЛОР НЕ СРАБОТАЕТ:
=== LORE START ===
{{LORE}}
=== LORE END ===
<img width="753" height="583" alt="image" src="https://github.com/user-attachments/assets/3b74a6ba-79b5-4c19-8a1f-7e4f1570247d" />
15) Настройка самой области
Потяните за любой уголок что бы расстянуть/стянуть область.
Потяните за рамку, что бы перенести область
CTRL+ALT+N - создать новую область
CTRL+ALT+D - разделить область на две, область захвата (оранжевая) и область перевода (синяя). И так же что бы соединить обратно
...
Пока что это все настройки что есть, возможно что-то добавится, а что уберется.
После всех настроек и изменения области под себя, жмите кнопку "Начать перевод", через некоторое время, если все сделали правильно, появится перевод текста.
...
2) Возиожность сменить сервер. Сейчас используется llama так как для меня это самое простое решение и в ней есть модели "все в одном"
3) Больше настроек для комофрта (настройки рамок, более приятный интрефейс и т.д)
...
instant — моментально; smooth — печать по символам (имитация новел и гач).
